...
const newDecompositionClasses: { [_: string]: string } = {
...
const newBidirectionalClasses: { [_: string]: string } = {
  AL: 'Arabic Letter',
  AN: 'Arabic Number',
...
const newCombiningClasses: { [_: string]: string } = {
  0: 'Not Reordered',
  1: 'Overlay',
  6: '',
  7: 'Nukta',
...
const newCategories: { [_: string]: string } = {
  Cc: 'Control',
  Cf: 'Format',
  Co: 'Private Use',
  Cs: 'Surrrogate',
...
function mapCsvToNewCodePoint(a: string[]): NewCodePoint {
  return {
    codePoint: a[0],
    characterName: a[1],
    generalCategory: a[2],
    canonicalCombiningClasses: a[3],
    bidirectionalCategory: a[4],
    characterDecompositionMapping: a[5],
    decimalDigitValue: a[6],
    digitValue: a[7],
    numericValue: a[8],
    mirrored: a[9],
    unicode1Name: a[10],
    commentField: a[11],
    uppercaseMapping: a[12],
    lowercaseMapping: a[13],
    titlecaseMapping: a[14],
  };
}
...
export function getCodePoints(search: Search, size = 128): Page<CodePoint> {
  ...
  const onSearch = (s: string[]): boolean => {
    return new RegExp(`${search?.search}`, 'i').test(s[1]);
  };

  const slice2 = search.search ? unicode.filter(onSearch) : unicode;

  const pageCount = Math.ceil(slice2.length / size);
  const pageStart = size * (page - 1);
  const pageEnd = size * page;

  const slice = slice2.slice(pageStart, pageEnd).map(mapCsvToNewCodePoint).map(mapNewCodePointToCodePoint);
...
export function mapNewCodePointToCodePoint(codePoint: NewCodePoint): CodePoint {
  return {
    id: Number.parseInt(codePoint.codePoint, 16),
    key: Number.parseInt(codePoint.codePoint, 16),
    name: codePoint.characterName,
    value: `U+${Number.parseInt(codePoint.codePoint, 16).toString(16).toUpperCase().padStart(4, '0')}`,
    block: getBlock(Number.parseInt(codePoint.codePoint, 16)),
    plane: getPlane(Number.parseInt(codePoint.codePoint, 16)),
    category: newCategories[codePoint.generalCategory],
    combiningClass: newCombiningClasses[codePoint.canonicalCombiningClasses],
    bidirectionalClass: newBidirectionalClasses[codePoint.bidirectionalCategory],
    decompositionClass: newDecompositionClasses[codePoint.characterDecompositionMapping.match(/^<(\w+)>.*$/)?.[1] ?? ''] ?? 'Canonical',
  };
}
